...
class Buffer(GObject.GBoxed):
    ...
    def map(self, flags: MapFlags) -> MapResult: ...
    def map_range(
...
# there is a strange dual behavior of Buffer.map() returning [bool, MapInfo] when called normally, but in a context
# manager it yields only MapInfo
class MapResult(tuple[bool, MapInfo]):
    def __enter__(self) -> MapInfo: ...
